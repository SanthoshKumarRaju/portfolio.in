...
window.PDATA = [
  { id:'linux', label:'Linux', icon:'🐧', projects:[
    { id:'basic-commands', title:'Basic Commands', env:'local',
      ...
      code:{ fname:'commands.sh', lang:'bash', body:`<span class="kw">pwd</span>                         <span class="cm"># print working directory</span>
<span class="kw">ls</span> -lah                     <span class="cm"># long list including hidden files</span>
<span class="kw">find</span> / -name <span class="str">"*.conf"</span> 2>/dev/null
<span class="kw">chmod</span> <span class="num">755</span> script.sh          <span class="cm"># rwxr-xr-x</span>
<span class="kw">chown</span> deploy:deploy /var/app
<span class="kw">ps</span> aux | <span class="kw">grep</span> nginx
<span class="kw">systemctl</span> status nginx
<span class="kw">journalctl</span> -u nginx -f
...
echo <span class="str">"✓ Backup ${DATE} uploaded"</span>` },
    ...
    { id:'user-management', title:'User Management', env:'server',
      ...
      code:{ fname:'user-setup.sh', lang:'bash', body:`<span class="kw">useradd</span> -m -s /bin/bash deploy
<span class="kw">usermod</span> -aG sudo deploy
<span class="kw">mkdir</span> -p /home/deploy/.ssh
<span class="kw">chmod</span> <span class="num">700</span> /home/deploy/.ssh
echo <span class="str">"ssh-rsa AAAA..."</span> &gt;&gt; /home/deploy/.ssh/authorized_keys
<span class="kw">chmod</span> <span class="num">600</span> /home/deploy/.ssh/authorized_keys
<span class="kw">chown</span> -R deploy:deploy /home/deploy/.ssh` },
      meta:[{k:'OS',v:'Ubuntu 22.04'},{k:'Where',v:'Production Server'},{k:'Status',v:'✓ Done',g:true}] }
  ]},

  { id:'jenkins', label:'Jenkins', icon:'🔧', projects:[
...
<span class="kw">volumes</span>:
  jenkins_home:` },
      meta:[{k:'Where',v:'Localhost'},{k:'Port',v:':8080'},{k:'Image',v:'LTS JDK17'},{k:'Status',v:'✓ Done',g:true}] },

    { id:'declarative-pipeline', title:'Declarative Pipeline', env:'local',
      page:'../../projects/jenkins/declarative-pipeline/index.html',
      desc:'Full declarative Jenkinsfile pipeline with stages: Checkout → Build → Test → Docker Build → Push → Deploy with Blue Ocean dashboard.',
      steps:['Create a <strong>Jenkinsfile</strong> in the root of your Git repo','Define stages as shown below','Jenkins: New Item → Pipeline → Pipeline from SCM','Point to your repo and Jenkinsfile path','Trigger a build and watch Blue Ocean'],
  ...
  <span class="kw">stages</span> {
    ...
    <span class="kw">stage</span>(<span class="str">'Deploy'</span>) {
      ...
      meta:[{k:'Where',v:'Localhost'},{k:'Trigger',v:'Git push'},{k:'Stages',v:'5'},{k:'Status',v:'✓ Done',g:true}] },
    ...
    { id:'gitlab-webhook', title:'GitLab Webhook', env:'server',
      ...
      steps:['Install GitLab Plugin in Jenkins → Manage Plugins','Create an API token in Jenkins for GitLab auth','GitLab: Settings → Webhooks → Add Jenkins URL','Set trigger events: Push + Merge Requests','Test the webhook and verify the build fires'],
...
<span class="kw">curl</span> -X POST <span class="str">"${JENKINS_URL}/job/my-pipeline/build"</span> \\
  ...
  { id:'docker', label:'Docker', icon:'🐳', projects:[
    { id:'sample-compose-app', title:'Sample Compose App', env:'local',
      ...
      steps:['Clone repo and cd into project directory','Copy <strong>.env.example</strong> to <strong>.env</strong> and fill values','Run <strong>docker compose up --build -d</strong>','Frontend: http://localhost:3000  · API: http://localhost:4000/api','Logs: <strong>docker compose logs -f api</strong>'],
      code:{ fname:'docker-compose.yml', lang:'yaml', body:`<span class="kw">version</span>: <span class="str">'3.8'</span>
<span class="kw">services</span>:
...
<span class="kw">volumes</span>: {pg_data:}` },
      meta:[{k:'Where',v:'Localhost'},{k:'Stack',v:'React+Node+PG'},{k:'Status',v:'✓ Done',g:true}] },
    ...
    { id:'dockerfile-best-practices', title:'Dockerfile Best Practices', env:'local',
      ...
      desc:'Multi-stage Dockerfile for a Node.js app using builder pattern — build stage installs devDeps, production stage copies only dist. Final image under 120 MB.',
      ...
      code:{ fname:'Dockerfile', lang:'bash', body:`<span class="cm"># ── Stage 1: Build ──────────────────</span>
...
<span class="kw">aws</span> cloudfront create-invalidation \\
  ...
  --paths <span class="str">"/*"</span>` },
      meta:[{k:'Services',v:'S3+CloudFront+R53'},{k:'Cost',v:'~$0 free tier'},{k:'Region',v:'ap-south-1'},{k:'Status',v:'✓ Done',g:true}] },

    { id:'ec2-setup', title:'EC2 Setup', env:'free',
      page:'../../projects/aws/ec2-setup/index.html',
      desc:'Launched and configured a t2.micro EC2 instance — Security Groups, Elastic IP, SSH key pair, user-data bootstrap and IAM instance profile for S3 access.',
...
echo <span class="str">"Bootstrap complete"</span> &gt; /tmp/done` },
      meta:[{k:'Instance',v:'t2.micro'},{k:'OS',v:'Ubuntu 22.04'},{k:'Region',v:'ap-south-1'},{k:'Status',v:'✓ Done',g:true}] },
...
Public RT  → igw-xxxxxxxx
Private RT → nat-xxxxxxxx` },
      meta:[{k:'CIDR',v:'10.0.0.0/16'},{k:'AZs',v:'ap-south-1a/b'},{k:'Subnets',v:'4'},{k:'Status',v:'✓ Done',g:true}] }
  ]},
...
<span class="kw">networks</span>:
  <span class="vr">mon-net</span>: {driver: overlay}` },
      meta:[{k:'Where',v:'Localhost'},{k:'Mode',v:'Swarm Manager'},{k:'Status',v:'✓ Done',g:true}] }
  ]},
...
<span class="kw">spec</span>:
  ...
  <span class="kw">template</span>:
    ...
    <span class="kw">spec</span>:
      ...
      meta:[{k:'Where',v:'Localhost'},{k:'Driver',v:'Docker'},{k:'K8s',v:'v1.28'},{k:'Status',v:'✓ Done',g:true}] },

    { id:'pod-deployment', title:'Pod Deployments', env:'local',
      page:'../../projects/kubernetes/pod-deployment/index.html',
      desc:'Hands-on with Kubernetes primitives — rolling updates, rollbacks, HorizontalPodAutoscaler and liveness/readiness probes.',
...
<span class="kw">spec</span>:
  ...
  <span class="vr">kibana</span>:
    ...
    <span class="kw">depends_on</span>: [elasticsearch]` },
      meta:[{k:'Where',v:'Localhost'},{k:'Stack',v:'8.11.0'},{k:'Kibana',v:':5601'},{k:'Status',v:'✓ Done',g:true}] }
  ]},

  { id:'grafana', label:'Grafana', icon:'📊', projects:[
    { id:'prometheus-setup', title:'Prometheus + Grafana', env:'local',
      ...
      steps:['Run node-exporter on each host to expose /metrics','Write prometheus.yml with scrape targets','Deploy stack with Docker Compose','Open Grafana at <strong>http://localhost:3000</strong> (admin/admin)','Add Prometheus data source → Import dashboard ID <strong>1860</strong>','Create custom alert rules in Alertmanager'],
...
<span class="kw">scrape_configs</span>:
  ...
  - <span class="kw">job_name</span>: <span class="str">'app'</span>
    <span class="kw">static_configs</span>:
      ...
      meta:[{k:'Where',v:'Localhost'},{k:'Grafana',v:':3000'},{k:'Prom',v:':9090'},{k:'Status',v:'✓ Done',g:true}] }
  ]},

  { id:'traefik', label:'Traefik', icon:'🔀', projects:[
    { id:'ssl-letsencrypt', title:'SSL + Let\'s Encrypt', env:'server',
      page:'../../projects/traefik/ssl-letsencrypt/index.html',
      desc:'Traefik as reverse proxy for all Docker services with automatic HTTPS via Let\'s Encrypt ACME. Docker label-based routing — no per-service manual config.',
      steps:['Create acme.json: <strong>touch acme.json &amp;&amp; chmod 600 acme.json</strong>','Write traefik.yml with entrypoints and certificatesResolvers','Add Traefik labels to each service','Point your domain DNS A-record to server IP','Deploy: docker compose up -d — Traefik auto-requests SSL certs'],
      code:{ fname:'docker-compose.yml', lang:'yaml', body:`<span class="kw">services</span>:
  ...
  <span class="vr">myapp</span>:
    ...
    <span class="kw">labels</span>:
      ...
      meta:[{k:'Where',v:'Server'},{k:'SSL',v:'Let\'s Encrypt'},{k:'Version',v:'v3.0'},{k:'Status',v:'✓ Done',g:true}] }
  ]},

  { id:'cicd', label:'CI/CD', icon:'🔄', projects:[
    { id:'gitlab-pipeline', title:'GitLab CI Pipeline', env:'server',
      page:'../../projects/cicd/gitlab-pipeline/index.html',
      desc:'End-to-end GitLab CI/CD pipeline: lint → test → docker build → push to registry → deploy to server via SSH. Manual gate for production with environment tracking.',
      steps:['Create <strong>.gitlab-ci.yml</strong> in repo root','Set GitLab CI/CD variables: REGISTRY_USER, REGISTRY_PASS, DEPLOY_HOST, DEPLOY_KEY','Push to main branch to trigger pipeline','Review stages in GitLab → CI/CD → Pipelines','Approve manual production deploy gate'],
...
<span class="vr">deploy-prod</span>:
  ...
  <span class="kw">script</span>:
    - ssh deploy@$DEPLOY_HOST "docker service update --image $IMAGE app_web"` },
      meta:[{k:'Where',v:'Server'},{k:'Trigger',v:'Git Push'},{k:'Gate',v:'Manual prod'},{k:'Status',v:'✓ Done',g:true}] }
  ]},

  { id:'vpc', label:'VPC & Networking', icon:'🛡️', projects:[
    { id:'basic-vpc', title:'Basic VPC Setup', env:'server',
      page:'../../projects/vpc/basic-vpc/index.html',
      desc:'Custom multi-tier AWS VPC from scratch — Internet Gateway, route tables, NAT Gateway, Security Groups for web/app/db tiers and Network ACLs for subnet-level security.',
      steps:['Create VPC with CIDR 10.0.0.0/16','Create public subnets in 2 AZs — enable auto-assign public IP','Create private subnets in same AZs','Create and attach Internet Gateway','Allocate Elastic IP → Create NAT Gateway in public subnet','Create route tables: public→IGW, private→NAT','Create Security Groups per tier'],
      code:{ fname:'sg-rules.sh', lang:'bash', body:`<span class="cm"># Web tier SG — public facing</span>
<span class="kw">aws</span> ec2 authorize-security-group-ingress \\
  --group-id sg-web --protocol tcp \\
  --port 443 --cidr 0.0.0.0/0

<span class="cm"># App tier SG — from web SG only</span>
<span class="kw">aws</span> ec2 authorize-security-group-ingress \\
  --group-id sg-app --protocol tcp \\
  --port 3000 --source-group sg-web

<span class="cm"># DB tier SG — from app SG only</span>
<span class="kw">aws</span> ec2 authorize-security-group-ingress \\
